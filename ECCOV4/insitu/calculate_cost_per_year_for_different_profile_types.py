...
for prof_type in profs.keys():
    ...
    prof_costs_T = np.zeros((num_years,1))
    prof_costs_S = np.zeros((num_years,1))
    prof_count_T = np.zeros((num_years,1))
    prof_count_S = np.zeros((num_years,1))
    ...
    for ptfgi in prof_type_file_globs:
        ...
        for f in files:
            ...
            for yi in range(num_years):
                ...
                # find the indices of the profiles that fall within cur_year
                ix= np.isin(prof_years, cur_year)
        
                # continue if at least one profile is within this cur_year
                if sum(ix) > 0:
                    print('found profiles of year ' + str(cur_year) + ' in file ' + f)
                    ...
                    if has_T_data:
                        ...
                        prof_costs_T[yi] += np.nansum(T_cost)
                        prof_count_T[yi] += T_data_count
                    ...
                    if has_S_data:
                        ...
                        prof_costs_S[yi] += np.nansum(S_cost)
                        prof_count_S[yi] += S_data_count
